...
export default function FunderPortal() {
  ...
  // Stats data
  const stats = [
    { label: 'Total Funders', value: funders?.length || 0, icon: <Users className="h-5 w-5 text-blue-500" /> },
    { label: 'Active Funders', value: funders?.filter(f => f.status === 'active').length || 0, icon: <CheckCircle className="h-5 w-5 text-green-500" /> },
    { label: 'Linked Projects', value: projects ? new Set(projects.map(p => p.funderId)).size : 0, icon: <Link2 className="h-5 w-5 text-purple-500" /> },
              ...
              {filteredFunders.length > 0 ? (
                filteredFunders.map((funder) => (
                  <tr key={funder.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-500 dark:text-gray-400">
                        {projects?.filter(p => p.funderId === funder.id).length || 0}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900 dark:text-white">
