...
export default function ProjectExpensesStackedBar({ className = '', timeRange = 'All', compact = false }) {
  ...
  const data = useMemo(() => {
    ...
    // group by project -> per-category totals
    const map = new Map(); // projectId -> { name, [category]: amount }
    ...
    posted.forEach((e) => {
      ...
      if (!map.has(projectKey)) {
        const base = { name: projectName };
        categoriesList.forEach((c) => (base[c] = 0));
        map.set(projectKey, base);
      }
      const entry = map.get(projectKey);
      entry[cat] = (entry[cat] || 0) + amount;
    });

    const rows = [];
    for (const [, entry] of map.entries()) {
      const row = { project: entry.name };
    ...
    return { rows, categoriesList };
  }, [expenses, projects, orgs, activeOrgId, timeRange]);

  // Legend interactivity: hooks must be declared before any early return
  const [hiddenCategories, setHiddenCategories] = useState(new Set());
  const [showAllLegend, setShowAllLegend] = useState(false);
  const categoriesAll = data.categoriesList || [];
  ...
  if (!(data && data.rows && data.rows.length)) {
    return (
      <div className={`bg-white p-4 rounded-2xl shadow-sm border border-gray-100 ${className} dark:bg-slate-900/70 dark:border-slate-800 dark:text-slate-100`}>
        <h3 className="text-lg font-semibold mb-2 text-gray-700 dark:text-slate-100">Project Expenses</h3>
  ...
  const categoryColors = (data.categoriesList || []).reduce((acc, c, i) => { acc[c] = colors[i % colors.length]; return acc; }, {});
    ...
    <div className={`overflow-hidden rounded-3xl bg-gradient-to-b from-white/95 to-white/80 backdrop-blur-xl ring-1 ring-gray-200 shadow-sm hover:shadow-md transition-shadow ${className} dark:from-slate-900/80 dark:to-slate-900/60 dark:ring-slate-800 dark:text-slate-100`}>
      <div className="px-5 pt-5 pb-3 flex items-center justify-between border-b border-slate-100/70 dark:border-slate-800">
        <div className="flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-blue-500/10 text-blue-600 dark:text-blue-300 flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3v18h18M7 13h6m-6-4h10m-10 8h4"/></svg>
          </div>
          <div>
            <h3 className="text-base md:text-lg font-semibold text-slate-900 dark:text-slate-100">Project Expenses</h3>
      ...
      <div className="px-5 py-4">
        <div className={`${compact ? 'h-36 lg:h-44' : 'h-48 lg:h-60'} overflow-x-auto`}>
          <div style={{ minWidth }} className="h-full lg:max-w-4xl">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.rows} margin={{ top: 10, right: 8, bottom: 6, left: 8 }} barCategoryGap="10%" barGap={6}>
                {/* gradients for each category to give modern look */}
                <defs>
                  ...
                  <filter id="barShadowP" x="-20%" y="-20%" width="140%" height="140%">
