...
if (!getApps().length) {
  try {
    ...
    if (isDev && import.meta.env.VITE_USE_FIREBASE_EMULATORS === 'true') {
    ...
    if (error && (error.code === 'auth/invalid-api-key' || (error.message && error.message.includes('invalid-api-key')))) {
      ...
      console.error('- The API key is incorrect or missing in .env (VITE_FIREBASE_API_KEY)');
      console.error('- The dev server was not restarted after updating .env');
      console.error('- You are running a different app instance that does not load .env');
      console.error('Actions to fix:');
      console.error('1) Verify .env contains the correct VITE_FIREBASE_* values.');
      console.error('2) Restart the dev server so Vite picks up the env changes.');
